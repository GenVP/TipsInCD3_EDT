...
Процедура ОбработатьГруппуОбщихМодулей(ПараметрыЗагрузки, ЗагружатьМетодыНеГлобальныхМодулей = Ложь) Экспорт
	...
	//+КД3
	КД3_ЗагрузкаМетаданных.ИнициализироватьПрогресс(ПараметрыЗагрузки, ВложенныеФайлы.Количество());
	//-КД3
	...
	Для Каждого ВложенныйФайл Из ВложенныеФайлы Цикл
		//+КД3
		КД3_ЗагрузкаМетаданных.СообщитьПрогресс(ПараметрыЗагрузки, "КД3_ОбщиеМодули");
		//-КД3
...
Процедура ИнициализироватьПрогресс(ОбщиеПеременные, КоличествоОбъектов, КД3_ПрогрессПоток = Неопределено) Экспорт
	
	Если НЕ ОбщиеПеременные.Свойство("КД3_СообщатьПрогресс") Тогда
		Возврат;
	КонецЕсли;
	
	Поток = ?(КД3_ПрогрессПоток = Неопределено, ОбщиеПеременные.КД3_ПрогрессПоток, КД3_ПрогрессПоток);
	
	ОбщиеПеременные.Вставить("КД3_ПрогрессТекущий" + Поток, 0); // Текущий прогресс с точностью до объекта
	ОбщиеПеременные.Вставить("КД3_ПрогрессПроцент" + Поток, 0); // Процент прогресса для вывода на индикаторе
	ОбщиеПеременные.Вставить("КД3_ПрогрессШаг" + Поток, 100 / КоличествоОбъектов); // Шаг увеличения процента прогресса
	
КонецПроцедуры

Процедура СообщитьПрогресс(ОбщиеПеременные, ПрогрессТекст, КД3_ПрогрессПоток = Неопределено, КД3_ПрогрессПроцент = Неопределено) Экспорт
	...
	Если КД3_ПрогрессПроцент = Неопределено Тогда
		// Увеличение текущего прогресса на шаг и рассчет изменения процента прогресса
		КД3_ПрогрессТекущий = ОбщиеПеременные["КД3_ПрогрессТекущий" + Поток] + 1;
		ОбщиеПеременные.Вставить("КД3_ПрогрессТекущий" + Поток, КД3_ПрогрессТекущий);
		КД3_ПрогрессПроцент = Окр(КД3_ПрогрессТекущий * ОбщиеПеременные["КД3_ПрогрессШаг" + Поток], 0);
		Если КД3_ПрогрессПроцент = ОбщиеПеременные["КД3_ПрогрессПроцент" + Поток] Тогда
			Возврат; // Процент прогресса не изменился
		КонецЕсли;
	КонецЕсли;
	ОбщиеПеременные.Вставить("КД3_ПрогрессПроцент" + Поток, КД3_ПрогрессПроцент);
	
	ДанныеПрогресса = Новый Структура;
	ДанныеПрогресса.Вставить("Поток", Поток);
	ДанныеПрогресса.Вставить("Текст", ПрогрессТекст);
	ДанныеПрогресса.Вставить("Процент", КД3_ПрогрессПроцент);
	ДанныеПрогресса.Вставить("Видимость", Истина);
	
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = "{КД3}" + ОбщегоНазначения.ЗначениеВСтрокуXML(ДанныеПрогресса);
	Сообщение.Сообщить();
	
КонецПроцедуры

Процедура ЗавершитьПрогресс(ОбщиеПеременные, КД3_ПрогрессПоток = Неопределено) Экспорт
	...
	ДанныеПрогресса = Новый Структура;
	ДанныеПрогресса.Вставить("Поток", Поток);
	ДанныеПрогресса.Вставить("Видимость", Ложь);
	
	ОбщегоНазначения.СообщитьПользователю("{КД3}" + ОбщегоНазначения.ЗначениеВСтрокуXML(ДанныеПрогресса));
	
КонецПроцедуры

Процедура ОбновитьПрогрессНаФорме(ИдентификаторЗадания, Форма) Экспорт
	
	ДанныеПрогресса = Новый Соответствие;
	Задание = ФоновыеЗадания.НайтиПоУникальномуИдентификатору(ИдентификаторЗадания);
	
	Если Задание <> Неопределено Тогда
		//ОповещенияСервера = СерверныеОповещения.СерверныеОповещенияДляКлиента(ИдентификаторЗадания, Неопределено);
		//Для Каждого СтруктураСообщения Из ОповещенияСервера Цикл
		//	Для Каждого Сообщение Из СтруктураСообщения.Содержимое.Результат.Результат.Сообщения Цикл
		//		Если СтрНачинаетсяС(Сообщение.Текст, "{КД3}") Тогда
		//			ДанныеПрогрессаПотока = ОбщегоНазначения.ЗначениеИзСтрокиXML(Сред(Сообщение.Текст, 6));
		//			ДанныеПрогресса.Вставить(ДанныеПрогрессаПотока.Поток, ДанныеПрогрессаПотока);
		//		КонецЕсли;
		//	КонецЦикла;
		//КонецЦикла;
	...
	Для Каждого КлючИЗначение Из ДанныеПрогресса Цикл
		ДанныеПотока = КлючИЗначение.Значение;
		Если КлючИЗначение.Ключ = 0 Тогда
			Форма.КД3_ПрогрессТекст0 = ДанныеПотока.Текст;
			Форма.КД3_Прогресс0 = ДанныеПотока.Процент;
			Форма.Элементы.КД3_Прогресс0.Видимость = Истина;
		Иначе
			ЭлементПрогресса = Форма.Элементы["КД3_Прогресс" + КлючИЗначение.Ключ];
			ЭлементПрогресса.Видимость = ДанныеПотока.Видимость;
			Если ДанныеПотока.Видимость Тогда
				Если ПустаяСтрока(ДанныеПотока.Текст) Тогда
					ЭлементПрогресса.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
				Иначе
					ЭлементПрогресса.Заголовок = ДанныеПотока.Текст;
					ЭлементПрогресса.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
				КонецЕсли;
				Форма["КД3_Прогресс" + КлючИЗначение.Ключ] = ДанныеПотока.Процент;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
