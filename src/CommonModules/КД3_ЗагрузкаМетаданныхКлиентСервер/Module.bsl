Процедура ПодготовитьФайлыДляЗагрузки(ПараметрыЗагрузки) Экспорт
	...
	Если НЕ ПараметрыЗагрузки.КД3_ЭтоСервер Тогда
		ПараметрыЗагрузки.Вставить("ПомещаемыеФайлы", Новый Соответствие);
	КонецЕсли;
	...
	Если ФайлСведений.Количество() = 0 Тогда
		...
		Возврат;
	КонецЕсли;
	ДобавитьФайлДляОбработки(ПараметрыЗагрузки, ПараметрыЗагрузки.КаталогЗагрузки, ФайлСведений[0].ПолноеИмя);
	...
	Если НЕ ПараметрыЗагрузки.ТолькоОбновитьПланыОбмена Тогда
		...
		Для Каждого ВложенныйФайл Из ВложенныеФайлы Цикл
			Если ВложенныйФайл.ЭтоКаталог() Тогда
				Продолжить;
			КонецЕсли;
			ДобавитьФайлДляОбработки(ПараметрыЗагрузки, КаталогЗагрузки, ВложенныйФайл.ПолноеИмя);
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого ВложенныйФайл Из ВложенныеФайлы Цикл
		...
		Если ПараметрыЗагрузки.ИсточникДанных = 0 Тогда
			...
			Если ФайлСПредопределенными.Количество() > 0 Тогда
				ДобавитьФайлДляОбработки(ПараметрыЗагрузки, КаталогЗагрузки, ФайлСПредопределенными[0].ПолноеИмя);
			КонецЕсли;
			// Состав плана обмена
			Если ИмяКаталога = "ExchangePlans" Тогда
				...
				Если ФайлССоставом.Количество() > 0 Тогда
					ДобавитьФайлДляОбработки(ПараметрыЗагрузки, КаталогЗагрузки, ФайлССоставом[0].ПолноеИмя);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		//+КД3
		Если ПараметрыЗагрузки.КД3_ИспользоватьКонтекстнуюПодсказку Тогда
			Если ПараметрыЗагрузки.КД3_ЗагружатьМетодыМодулей Тогда
				...
				Для Каждого ФайлСМодулем Из ФайлыСМодулем Цикл
					ДобавитьФайлДляОбработки(ПараметрыЗагрузки, КаталогЗагрузки, ФайлСМодулем.ПолноеИмя);
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
		//-КД3
...
Процедура ДобавитьФайлДляОбработки(ПараметрыЗагрузки, КаталогЗагрузки, ПолноеИмяФайла)
	
	Если ПараметрыЗагрузки.КД3_ЭтоСервер Тогда
		ПараметрыЗагрузки.ПомещенныеФайлы.Добавить(Новый Структура("ПолноеИмя, Хранение, ЭтоРасширение", ПолноеИмяФайла, ПолноеИмяФайла, ПараметрыЗагрузки.ЭтоРасширение));
	Иначе
		ПомещаемыеФайлы = ПараметрыЗагрузки.ПомещаемыеФайлы[КаталогЗагрузки];
		Если ПомещаемыеФайлы = Неопределено Тогда
			ПомещаемыеФайлы = Новый Структура("Файлы,ЭтоРасширение", Новый Массив, ПараметрыЗагрузки.ЭтоРасширение);
			ПараметрыЗагрузки.ПомещаемыеФайлы[КаталогЗагрузки] = ПомещаемыеФайлы;
		КонецЕсли;
		ПомещаемыеФайлы.Файлы.Добавить(Новый ОписаниеПередаваемогоФайла(ПолноеИмяФайла));
	КонецЕсли;
