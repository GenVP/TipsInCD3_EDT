...
Процедура ВыполнитьЗагрузкуМетаданных(ОбщиеПеременные, АдресВременногоХранилища) Экспорт
	...
	//+КД3
	КД3_ИнициализироватьПрогресс(ОбщиеПеременные);
	//-КД3
	...
	Если ОбщиеПеременные.КД3_ИспользоватьКонтекстнуюПодсказку Тогда
		...
		Иначе
			...
			Если НастройкиКонфигурации.МестоХраненияИндексов <> ОбщиеПеременные.КД3_МестоХраненияИндексов
			 ИЛИ НастройкиКонфигурации.КаталогИндексов <> ОбщиеПеременные.КД3_КаталогИндексов Тогда
				НастройкиКонфигурации.МестоХраненияИндексов = ОбщиеПеременные.КД3_МестоХраненияИндексов;
				НастройкиКонфигурации.КаталогИндексов = ОбщиеПеременные.КД3_КаталогИндексов;
				КД3_Метаданные.ИзменитьНастройкиКонфигурации(ОбщиеПеременные.Конфигурация, НастройкиКонфигурации);
			КонецЕсли;
		...
		// Подключение внешней компонены для работы с регулярными выражениями
		Если ОбщиеПеременные.КД3_ЗагружатьМетодыМодулей Тогда
			КД3_ЗагрузкаМетаданных.ПодключитьВычислитель(ОбщиеПеременные);
		КонецЕсли;
	КонецЕсли;
	//-КД3
	// Начало загрузки.
	...
	//+КД3
	КД3_СообщитьПрогресс(ОбщиеПеременные, "Получение существующих объектов");
	//-КД3
	...
	//+КД3
	КД3_СообщитьПрогресс(ОбщиеПеременные, "Заполнение типов свойств");
	//-КД3
	...
	//+КД3
	КД3_СообщитьПрогресс(ОбщиеПеременные, "Заполнение типов регистраторов");
	//-КД3
	...
	//+КД3
	КД3_СообщитьПрогресс(ОбщиеПеременные, "Пометка на удаление лишних объектов");
	//-КД3
	...
	КонецЦикла;
	//+КД3
	КД3_СообщитьПрогресс(ОбщиеПеременные, "Удаление временных файлов");
	//-КД3
	//+КД3
	...
	Если ОбщиеПеременные.КД3_ИспользоватьКонтекстнуюПодсказку И ОбщиеПеременные.КД3_МестоХраненияИндексов <> 0 Тогда
		// Сохранение индексов метаданных (отчетов, обработок, общих модулей)
...
Процедура ОбработатьКаталогиСДанными(ОбщиеПеременные)
	//+КД3
	ОбщиеПеременные.Вставить("КД3_ТекущаяГруппа", 0); // Для прогресса
	//-КД3
	Если ОбщиеПеременные.ТолькоОбновитьПланыОбмена Тогда
	...
	//+КД3
	Если НЕ ОбщиеПеременные.ТолькоОбновитьПланыОбмена И ОбщиеПеременные.КД3_ИспользоватьКонтекстнуюПодсказку И ОбщиеПеременные.КД3_МестоХраненияИндексов <> 0 Тогда
...
Процедура ОбработатьГруппуОбъектов(ОбщиеПеременные, ИмяГруппы, ИмяКаталога, ПрефиксИменЭлементов, ТипОбъекта)
	...
	КонецЕсли;
	//+КД3
	КД3_СообщитьПрогрессГруппы(ОбщиеПеременные, ИмяГруппы, ВложенныеФайлы.Количество());
	//-КД3
...
Функция НайтиПомещенныеФайлыПоИмени(ПомещенныеФайлы, ИмяФайла, ОбщиеПеременные)
	//+КД3 Прямой поиск файлов на сервере
	Если ОбщиеПеременные.КД3_ЭтоСервер Тогда
		Если СтрНачинаетсяС(ИмяФайла, ОбщиеПеременные.КаталогЗагрузки) Тогда
			НайденныеФайлы = НайтиФайлы(ИмяФайла);
		Иначе
			НайденныеФайлы = НайтиФайлы(ОбщиеПеременные.КаталогЗагрузки, ИмяФайла, Истина);
		КонецЕсли;
...
Функция НайтиПомещенныеФайлыПоМаске(ПомещенныеФайлы, ВложенныйКаталог, РасширениеФайла, ОбщиеПеременные)
	//+КД3 Прямой поиск файлов на сервере
	Если ОбщиеПеременные.КД3_ЭтоСервер Тогда
		Возврат КД3_ЗагрузкаМетаданных.НайтиПомещенныеФайлыПоМаске(Неопределено, ВложенныйКаталог, РасширениеФайла, ОбщиеПеременные);
	КонецЕсли;
	//+КД3
...
Процедура КД3_ИнициализироватьПрогресс(ОбщиеПеременные)
	...
	КД3_ГруппНаКонфигурацию = ?(ОбщиеПеременные.ТолькоОбновитьПланыОбмена, 1, 16);
	ОбщиеПеременные.Вставить("КД3_ШагПрогресса", 100 / (6 - 1 + КД3_ГруппНаКонфигурацию +
		?(ОбщиеПеременные.ЕстьРасширения, КД3_ГруппНаКонфигурацию * ОбщиеПеременные.КаталогиРасширений.Количество(), 0)));
	ОбщиеПеременные.Вставить("КД3_ПрогрессТекущий", 0);
	ОбщиеПеременные.Вставить("КД3_ПрогрессШаблонГруппы", СтрШаблон("Обработка каталога данных %%1%%2 из %1 - %%3 (%%4)", КД3_ГруппНаКонфигурацию));
	
КонецПроцедуры

Процедура КД3_СообщитьПрогресс(ОбщиеПеременные, ПрогрессТекст)
	
	Если НЕ ОбщиеПеременные.Свойство("КД3_СообщатьПрогресс") Тогда
		Возврат;
	КонецЕсли;
	
	ОбщиеПеременные.КД3_ПрогрессТекущий = ОбщиеПеременные.КД3_ПрогрессТекущий + 1;
	ДлительныеОперации.СообщитьПрогресс(ОбщиеПеременные.КД3_ПрогрессТекущий * ОбщиеПеременные.КД3_ШагПрогресса, ПрогрессТекст);
	
КонецПроцедуры

Процедура КД3_СообщитьПрогрессГруппы(ОбщиеПеременные, ИмяГруппы, КоличествоФайлов)
	
	Если НЕ ОбщиеПеременные.Свойство("КД3_СообщатьПрогресс") Тогда
		Возврат;
	КонецЕсли;
	
	ОбщиеПеременные.КД3_ТекущаяГруппа = ОбщиеПеременные.КД3_ТекущаяГруппа + 1;
	ПрогрессТекст = СтрШаблон(ОбщиеПеременные.КД3_ПрогрессШаблонГруппы,
		?(ОбщиеПеременные.ЭтоРасширение, " расширения", ""), ОбщиеПеременные.КД3_ТекущаяГруппа, ИмяГруппы,
		СтрокаСЧислом(";%1 файл;;%1 файла;%1 файлов;%1 файла", КоличествоФайлов, ВидЧисловогоЗначения.Количественное));
	КД3_СообщитьПрогресс(ОбщиеПеременные, ПрогрессТекст);
