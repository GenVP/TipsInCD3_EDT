...
Процедура КД3_ИзменитьРежимОбработки()
	...
	Иначе
		КД3_КоличествоПотоков = ?(КД3_ЭтоСервер, Элементы.КД3_КоличествоПотоков.МаксимальноеЗначение, 1);
...
Процедура ПодготовитьФайлыДляСервераНачало()
	...
	Если КД3_ЭтоСервер Тогда
		// В режиме сервера поиск/чтение файлов выполняется во время загрузки
		ПараметрыЗагрузки.Вставить("ПомещенныеФайлы", Неопределено);
	Иначе
		КД3_ЗагрузкаМетаданныхКлиентСервер.ПодготовитьФайлыДляЗагрузки(ПараметрыЗагрузки);
	КонецЕсли;
	//-КД3
	...
	//+КД3
	КД3_Прогресс0 = 0;
	КД3_ПрогрессТекст0 = "Обработка файлов данных";
...
Функция ДлительнаяОперацияВыполнена()
	...
	Для Каждого КлючИЗначение Из ДанныеПрогресса Цикл
		...
		Если КлючИЗначение.Ключ = 0 Тогда
			КД3_ПрогрессТекст0 = ДанныеПотока.Текст;
			КД3_Прогресс0 = ДанныеПотока.Процент;
			Элементы.КД3_Прогресс0.Видимость = Истина;
		Иначе
			Элементы["КД3_Прогресс" + КлючИЗначение.Ключ].Видимость = ДанныеПотока.Видимость;
			Если ДанныеПотока.Видимость Тогда
				ЭлементПрогресса = Элементы["КД3_Прогресс" + КлючИЗначение.Ключ];
				...
				КонецЕсли;
				ЭтотОбъект["КД3_Прогресс" + КлючИЗначение.Ключ] = ДанныеПотока.Процент;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	//-КД3
