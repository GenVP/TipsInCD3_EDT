...
Процедура ВыполнитьЗагрузкуМетаданных(ОбщиеПеременные, АдресВременногоХранилища) Экспорт
	...
	Если КД3_ЗагрузкаМетаданных.НужноСообщатьПрогресс(ОбщиеПеременные) Тогда
		// 6 - общее количество вызовов УвеличитьСообщитьПрогрессОбщий в ВыполнитьЗагрузкуМетаданных()
		...
		КоличествоОбъектов = 7 + ГруппНаКонфигурацию + 1;
		...
		КонецЕсли;
		КД3_ЗагрузкаМетаданных.НачатьПрогрессОбщий(ОбщиеПеременные, КоличествоОбъектов);
	КонецЕсли;
	//-КД3
	...
	//+КД3
	КД3_ЗагрузкаМетаданных.УвеличитьСообщитьПрогрессОбщий(ОбщиеПеременные, "Получение существующих объектов");
	...
	//+КД3
	КД3_ЗагрузкаМетаданных.УвеличитьСообщитьПрогрессОбщий(ОбщиеПеременные, "Заполнение типов планов видов расчета");
	//-КД3
	...
	//+КД3
	КД3_ЗагрузкаМетаданных.УвеличитьСообщитьПрогрессОбщий(ОбщиеПеременные, "Заполнение типов свойств");
	...
	//+КД3
	КД3_ЗагрузкаМетаданных.УвеличитьСообщитьПрогрессОбщий(ОбщиеПеременные, "Привязка задач к бизнес-процессам");
	...
	//+КД3
	КД3_ЗагрузкаМетаданных.УвеличитьСообщитьПрогрессОбщий(ОбщиеПеременные, "Заполнение типов регистраторов");
	//-КД3
	...
	//+КД3
	КД3_ЗагрузкаМетаданных.УвеличитьСообщитьПрогрессОбщий(ОбщиеПеременные, "Пометка на удаление неактуальных объектов");
	Если НЕ ОбщиеПеременные.ЗагрузитьНовую Тогда
	...
	//+КД3
	КД3_ЗагрузкаМетаданных.УвеличитьСообщитьПрогрессОбщий(ОбщиеПеременные, "Удаление временных файлов");
	//-КД3
...
Процедура ОбработатьГруппуОбъектов(ОбщиеПеременные, ИмяГруппы, ИмяКаталога, ПрефиксИменЭлементов, ТипОбъекта)
	...
	КонецЕсли;
	//+КД3
	КД3_ЗагрузкаМетаданных.НачатьПрогрессРасшифровки(ОбщиеПеременные, ВложенныеФайлы.Количество(), ИмяГруппы);
	//-КД3
...
Процедура КД3_ОбработатьГруппыОбъектовМногопоточно(ОбщиеПеременные, ГруппыОбъектов)
	...
	Если КоличествоПотоков = 1 Тогда
		...
		Для Каждого ГруппаОбъектов Из ГруппыОбъектов Цикл
			...
			КД3_ЗагрузкаМетаданных.ЗавершитьПрогрессРасшифровки(ОбщиеПеременные, ПараметрыЗадания.ИмяГруппы);
		КонецЦикла;
	Иначе
		КлючКонфигурации = XMLСтрока(ОбщиеПеременные.Конфигурация);
		Таймаут = ?(КД3_ЗагрузкаМетаданных.НужноСообщатьПрогресс(ОбщиеПеременные), 5, 60);
		...
		Пока Истина Цикл
			...
			Пока Индекс >= 0 Цикл
				...
				Если Задание.Состояние = СостояниеФоновогоЗадания.Завершено Тогда
					...
					КД3_ЗагрузкаМетаданных.ЗавершитьПрогрессРасшифровки(ОбщиеПеременные, ГруппаОбъектов.ПараметрыЗадания.ИмяГруппы);
...
Процедура КД3_ОбработатьГруппуОбъектовМногопоточно(ПараметрыЗадания, АдресРезультата) Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВызватьИсключение СтрШаблон("КД3: Ошибка обработки %1: %2", ПараметрыЗадания.ТипОбъекта, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
