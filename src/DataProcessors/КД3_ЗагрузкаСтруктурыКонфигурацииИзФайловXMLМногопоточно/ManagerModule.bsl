...
Процедура ВыполнитьЗагрузкуМетаданных(ОбщиеПеременные, АдресВременногоХранилища) Экспорт
	//+КД3
	_ВремяНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ЗаписьЖурналаРегистрации("КД3_ТЕСТ", УровеньЖурналаРегистрации.Информация,,, "Начало");
	//+КД3
	...
	Если ОбщиеПеременные.КД3_ИспользоватьКонтекстнуюПодсказку Тогда
		...
		Если ОбщиеПеременные.КД3_КоличествоПотоков = 1 И ОбщиеПеременные.КД3_ЗагружатьМетодыМодулей Тогда
	...
	//+КД3
	//КД3_ОбработатьКоллекциюОбъектовМногопоточно(ОбщиеПеременные, "КД3_ЗаполнениеТиповСвойств", ОбщиеПеременные.ОтложенноеЗаполнениеТиповСвойств);
	...
	КонецЕсли;
	//+КД3
	_ВремяОкончания = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ЗаписьЖурналаРегистрации("КД3_ТЕСТ", УровеньЖурналаРегистрации.Информация,,, "Окончание: " + Окр((_ВремяОкончания - _ВремяНачала) / 1000 / 60, 2));
	//+КД3
...
Процедура ОбработатьКаталогиСДанными(ОбщиеПеременные)
	...
	ГруппыОбъектов = КД3_ГруппыДляОбработки();
...
Процедура КД3_ОбработатьКоллекциюОбъектовМногопоточно(ОбщиеПеременные, ИмяКоллекции, КоллекцияОбъектов)
	
КонецПроцедуры

Функция КД3_ГруппыДляОбработки()
...
Функция КД3_ДобавитьГруппуКОбработке(ГруппыОбъектов, ИмяГруппы, ИмяКаталога, ПрефиксИменЭлементов, ТипОбъекта)
...
Процедура КД3_ОбработатьГруппыОбъектовМногопоточно(ОбщиеПеременные, ГруппыОбъектов)
	...
	Если КоличествоПотоков = 1 Тогда
		...
		Для Каждого ГруппаОбъектов Из ГруппыОбъектов Цикл
			...
			// Инициализация прогресса потока группы
