...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ЗагрузитьКонсольКодаИзМакета.Заголовок = СтрШаблон("Из макета (версия %1)",
		Метаданные.ОбщиеМакеты.КД3_src.Комментарий); 
...
Процедура ЗагрузитьКонсольКодаИзМакета(Команда)
	КД3_Настройки.УстановитьВерсиюКонсолиКода(Неопределено);
...
&НаКлиенте
Процедура ЗагрузитьКонсольКодаИзGitHubDevelop(Команда)
	...
	КД3_Настройки.УстановитьВерсиюКонсолиКода(АдресФайлаЗагрузки, "github/salexdv/bsl_console/develop");
	КД3_РедакторКодаКлиент.УдалитьИсходники();
	ОбновитьИнформациюПоКонсолиКода();
...
Процедура ЗагрузитьКонсольКодаИзФайлаЗавершение(РезультатПомещенияФайлов, ДопПараметры) Экспорт
	...
	КД3_Настройки.УстановитьВерсиюКонсолиКода(АдресФайлаЗагрузки, РезультатПомещенияФайлов.Имя);
	КД3_РедакторКодаКлиент.УдалитьИсходники();
	ОбновитьИнформациюПоКонсолиКода();
...
Процедура ОбновитьИнформациюПоКонсолиКода()
	...
	ВерсияКонсолиКода = КД3_Настройки.ВерсияКонсолиКода();
	Элементы.КонсольКодаИнформация.Заголовок = ВерсияКонсолиКода.Информация + Символы.ПС + "Версия: " + ВерсияКонсолиКода.Версия;
КонецПроцедуры
